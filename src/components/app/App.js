...
export const App = () => {
  ...
  const lastGuessedNumber = guessedNumbers.length ? guessedNumbers[guessedNumbers.length - 1] : null;
  ...
  const higherNumbers = guessedNumbers.filter((number) => number > randomNumber);
  ...
  return (
    <>
    { hasWon ? <WinningScreen restart={restart} guesses={guessCounter} /> : (
    <div className="app">        
        <div className="wrapper">
        
          <Alert message={getMessage()} />
          <header className="header">
            <h1 id="headline">Ca<span>N</span> you gu<span>E</span>ss the mysteri<span>O</span>us number?</h1>
          </header>
          ...
          <div className="guessPanel">            
            <GuessForm value={value} onChangedValue={handleChangeValue} handleFormGuess={handleGuess} />            
          </div>
        </div>
        
        <div className="countPanel" style={{backgroundImage: `url(${'./images/numbers.jpg'})` }}>
